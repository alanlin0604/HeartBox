...
> **Pre-launch 狀態（2026-05-16）：** 標示為「[HIDDEN]」的模組已隱藏 UI（路由 redirect、入口下線），
> 後端 model / endpoint 保留以維持資料完整性與成就計數。
> 重啟步驟見 [TODO.md](../TODO.md)「諮商師功能反向恢復計畫」。
...
| 5 | 諮商師平台 `[HIDDEN]` | 6 | 申請審核、檔案管理、排班、預約、報價 — UI pre-launch 已隱藏 |
| 6 | 即時通訊與通知 | 5 | 對話管理、訊息收發、WebSocket、通知系統、筆記分享（諮商師分享部分隨 #5 隱藏） |
...
| 11 | 管理後台 | 4 | 系統統計、用戶管理、回饋管理（諮商師審核 Tab pre-launch 已隱藏） |
