...
#
# This file is ASCII-only on purpose: PowerShell 5.x on a zh-TW Windows
# defaults to CP950 when reading .ps1 files without a BOM, and UTF-8
# punctuation (em-dash, arrows, checkmarks) byte-mangles into bogus
# syntax tokens. Keep edits ASCII-only.
#
# WHY:
...
function Install-Nssm {
    ...
    if ($choco) {
        if (Confirm-Step "Install NSSM via Chocolatey (choco install nssm -y)?") {
            & choco install nssm -y --no-progress | Out-Host
            $found = Find-Nssm
            if ($found) { return $found }
        }
    }
    if (Confirm-Step "Download NSSM 2.24 directly to C:\Tools\nssm\?") {
        $tmp = "$env:TEMP\nssm-2.24.zip"
        ...
        if (Test-Path $exe) {
            ...
            Write-Host "    [OK] Installed to C:\Tools\nssm\win64\nssm.exe" -ForegroundColor Green
            return 'C:\Tools\nssm\win64\nssm.exe'
        }
    }
    Write-Host "[X] NSSM install aborted. Install manually from https://nssm.cc/ and re-run." -ForegroundColor Red
    exit 1
}
...
if ($Uninstall) {
    ...
    $nssm = Find-Nssm
    if (-not $nssm) {
        Write-Host "[X] nssm.exe not found. Remove manually:" -ForegroundColor Red
        Write-Host "      sc.exe stop $SERVICE_NAME"
        Write-Host "      sc.exe delete $SERVICE_NAME"
        exit 1
    }
    if (-not (Service-Exists $SERVICE_NAME)) {
        Write-Host "[i] Service $SERVICE_NAME not installed; nothing to do." -ForegroundColor Yellow
        exit 0
    }
    Write-Host "[i] Stopping $SERVICE_NAME ..." -ForegroundColor Yellow
...
if (-not (Test-Path $ENV_FILE)) {
    Write-Host "[X] $ENV_FILE not found." -ForegroundColor Red
    Write-Host "    Create it first per docs/PHASE0B-YOUR-CHECKLIST.md section 2:" -ForegroundColor Yellow
...
Write-Host "[OK] Found $ENV_FILE" -ForegroundColor Green

# Prereq 2: venv python
if (-not (Test-Path $VENV_PY)) {
    Write-Host "[X] Backend venv not found at $VENV_PY" -ForegroundColor Red
    Write-Host "    Run: cd backend; python -m venv venv; venv\Scripts\pip install -r ..\requirements.txt" -ForegroundColor Yellow
    exit 1
}
Write-Host "[OK] Found venv python" -ForegroundColor Green
...
Write-Host "[OK] NSSM at $nssm" -ForegroundColor Green
...
if (Service-Exists $SERVICE_NAME) {
    Write-Host ""
    Write-Host "[i] Service $SERVICE_NAME already exists." -ForegroundColor Yellow
    if (-not (Confirm-Step "    Stop + remove and reinstall fresh?")) {
...
Write-Host ""
Write-Host "[i] Installing service..." -ForegroundColor Yellow
...
# USERPROFILE is also needed so llm_server/config.py finds ~/.heartbox-llm.env.
# NSSM expects newline-separated KEY=value lines (not NUL-separated).
$envLines = @(
    "HF_HOME=$hfHome",
...
& $nssm set $SERVICE_NAME AppEnvironmentExtra ($envLines -join "`r`n") | Out-Null
...
# Graceful shutdown: SIGINT -> WM_CLOSE -> kill, each with 10s window
# so TAIDE model unload doesn't get truncated.
...
& $nssm set $SERVICE_NAME AppStdoutCreationDisposition 4 | Out-Null
...
& $nssm set $SERVICE_NAME AppRotateSeconds 86400 | Out-Null

# Boot-time start (before user login).
...
# ----------------------------------------------------------------------
Write-Host ""
Write-Host "[i] Starting service ..." -ForegroundColor Yellow
...
if ($ok) {
    Write-Host "[OK] /health responding at $HEALTH_URL after $elapsed s" -ForegroundColor Green
    Write-Host ""
    ...
    Write-Host "  nssm restart HeartBoxLLM"
    ...
    Write-Host "[X] /health did not respond within 120 s." -ForegroundColor Red
